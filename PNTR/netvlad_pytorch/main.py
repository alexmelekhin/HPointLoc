...
def test(eval_set, epoch=0, write_tboard=False):
    ...
    for i, prediction in enumerate(predictions):
        output[eval_set.images[eval_set.dbStruct.numDb+i]] = {}
        output[eval_set.images[eval_set.dbStruct.numDb+i]]['database_images'] = list([eval_set.images[j] for j in prediction])
        for j, num_db in enumerate(prediction):
            ...
            output[eval_set.images[eval_set.dbStruct.numDb+i]]['cos_metric'] = cosine_similarity(dbfeature, qfeature)[0][0]
            results_txt_file.write(eval_set.images[eval_set.dbStruct.numDb+i].split('/')[-1] + \
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        whole_test_set = dataset.get_whole_test_set(opt.metadata_json_file)
